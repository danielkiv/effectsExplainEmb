...
# --- Argument Parsing ---
parser = argparse.ArgumentParser(description="Run embedding experiments for a specific encoder.")
parser.add_argument('--encoder_index', type=int, required=True, help='Index of the encoder type to use.')
parser.add_argument('--num_repetitions', type=int, default=1, help='Number of times to repeat the experiment for the encoder.')
parser.add_argument('--base_experiment_dir', type=str, default='./results/dumb_multi_embedding_fixed2_repeated', help='Base directory for all experiment results.')
...
if args.encoder_index < 0 or args.encoder_index >= len(all_encoder_types):
    print(f"FATAL ERROR: Encoder index {args.encoder_index} is out of bounds for {len(all_encoder_types)} encoders.")
    exit()
...
print(f"Received Encoder Index: {args.encoder_index}, corresponding to Encoder: {encoder_to_run}")
...
for encoder in encoder_types: # This loop will run only once for the selected encoder_to_run
    ...
    for rep_num in range(1, args.num_repetitions + 1):
        ...
        REPETITION_DIR = os.path.join(BASE_EXPERIMENT_DIR, encoder, f"repetition_{rep_num}")
        ...
        try:
            ...
            if np.isnan(embeddings_np).any() or np.isinf(embeddings_np).any():
                print(f"Warning: NaN or Inf values found in embeddings for {encoder} on rep {rep_num}. Skipping this repetition.")
            ...
            print(f"Error generating embeddings for {encoder} on rep {rep_num}: {e}. Skipping this repetition.")
        ...
        X_train_gs, X_test_gs, y_train, y_test = train_test_split(
            X_for_geoshapley, y, test_size=0.20, random_state=current_random_state_split # Use varying state
        )
        ...
        mlp_model = MLPRegressor(random_state=rep_num, max_iter=600, hidden_layer_sizes=(64, 32), alpha=0.01, early_stopping=True) #
        mlp_failed = False
        try:
            ...
            current_rep_model_metrics['MLP'] = {'Train_R2': mlp_train_score, 'Test_R2': mlp_test_score}
            repetition_model_metrics_mlp.append({'Repetition': rep_num, 'Train_R2': mlp_train_score, 'Test_R2': mlp_test_score})
        except Exception as e:
            print(f"MLP model training failed for rep {rep_num}: {e}")
            mlp_failed = True
            current_rep_model_metrics['MLP'] = {'Train_R2': np.nan, 'Test_R2': np.nan}
            repetition_model_metrics_mlp.append({'Repetition': rep_num, 'Train_R2': np.nan, 'Test_R2': np.nan})
        
        # --- XGBoost Model ---
        print(f"\nTraining XGBoost model for {encoder}, repetition {rep_num}...")
        # Use a different random_state for XGBoost in each repetition
        xgb_model = XGBRegressor(random_state=rep_num, objective='reg:squarederror', n_estimators=100) #
        xgb_failed = False
        try:
            ...
            current_rep_model_metrics['XGBoost'] = {'Train_R2': xgb_train_score, 'Test_R2': xgb_test_score}
            repetition_model_metrics_xgb.append({'Repetition': rep_num, 'Train_R2': xgb_train_score, 'Test_R2': xgb_test_score})
        except Exception as e:
            print(f"XGBoost model training failed for rep {rep_num}: {e}")
            xgb_failed = True
            current_rep_model_metrics['XGBoost'] = {'Train_R2': np.nan, 'Test_R2': np.nan}
            repetition_model_metrics_xgb.append({'Repetition': rep_num, 'Train_R2': np.nan, 'Test_R2': np.nan})
        ...
        if not mlp_failed:
            ...
            try:
                ...
                try:
                    ...
                    plt.savefig(os.path.join(REPETITION_DIR, "mlp_geoshapley_summary_plot.pdf"), bbox_inches='tight'); plt.close('all')
                    summary_stats_mlp = mlp_rslt.summary_statistics(feature_names=geoshapley_feature_names[:-2])
                    summary_stats_mlp.to_csv(os.path.join(REPETITION_DIR, "mlp_geoshapley_summary_statistics.csv"))
                except Exception as e: print(f"Error in MLP summary plot/stats for rep {rep_num}: {e}"); plt.close('all')

                est_intercept_mlp = mlp_rslt.base_value + mlp_rslt.geo
                all_rep_est_intercept_mlp.append(est_intercept_mlp) # Store for aggregation
                ...
                if metrics: current_rep_spatial_metrics_mlp.append({**metrics, 'Repetition': rep_num})
            ...
            except Exception as e: print(f"Error during MLP GeoShapley for rep {rep_num}: {type(e).__name__}: {e}")
        ...
        if not xgb_failed:
            ...
            try:
                ...
                try:
                    ...
                    plt.savefig(os.path.join(REPETITION_DIR, "xgb_geoshapley_summary_plot.pdf"), bbox_inches='tight'); plt.close('all')
                    summary_stats_xgb = xgb_rslt.summary_statistics(feature_names=geoshapley_feature_names[:-2])
                    summary_stats_xgb.to_csv(os.path.join(REPETITION_DIR, "xgb_geoshapley_summary_statistics.csv"))
                except Exception as e: print(f"Error in XGBoost summary plot/stats for rep {rep_num}: {e}"); plt.close('all')
                ...
                if metrics: current_rep_spatial_metrics_xgb.append({**metrics, 'Repetition': rep_num})
                ...
                if xgb_svc_interactions_smooth is not None and xgb_svc_interactions_smooth.shape == (SIZE*SIZE, len(original_feature_indices)):
                    ...
                    if metrics: current_rep_spatial_metrics_xgb.append({**metrics, 'Repetition': rep_num})
            except Exception as e: print(f"Error during XGBoost GeoShapley for rep {rep_num}: {type(e).__name__}: {e}")
        repetition_spatial_metrics_xgb.extend(current_rep_spatial_metrics_xgb)

        print(f"----- Finished Repetition: {rep_num}/{args.num_repetitions} for Encoder: {encoder} -----")

    # --- Aggregation and Saving for the current ENCODER across all its repetitions ---
    ENCODER_RESULTS_DIR = os.path.join(BASE_EXPERIMENT_DIR, encoder) # Main folder for this encoder's aggregated results
    ...
    if repetition_model_metrics_mlp:
        ...
        mlp_metrics_summary = mlp_metrics_df.drop(columns=['Repetition']).agg(['mean', 'std', 'count'])
        mlp_metrics_df.to_csv(os.path.join(ENCODER_RESULTS_DIR, "mlp_model_performance_all_reps.csv"), index=False)
        mlp_metrics_summary.to_csv(os.path.join(ENCODER_RESULTS_DIR, "mlp_model_performance_summary.csv"))
    ...
    if repetition_model_metrics_xgb:
        ...
        xgb_metrics_summary = xgb_metrics_df.drop(columns=['Repetition']).agg(['mean', 'std', 'count'])
        xgb_metrics_df.to_csv(os.path.join(ENCODER_RESULTS_DIR, "xgb_model_performance_all_reps.csv"), index=False)
        xgb_metrics_summary.to_csv(os.path.join(ENCODER_RESULTS_DIR, "xgb_model_performance_summary.csv"))
    ...
    if repetition_spatial_metrics_mlp:
        ...
        mlp_spatial_summary = mlp_spatial_df.drop(columns=['Repetition', 'encoder', 'model']).groupby('spatial_effect').agg(['mean', 'std', 'count'])
        mlp_spatial_df.to_csv(os.path.join(ENCODER_RESULTS_DIR, "mlp_spatial_metrics_all_reps.csv"), index=False)
        mlp_spatial_summary.to_csv(os.path.join(ENCODER_RESULTS_DIR, "mlp_spatial_metrics_summary.csv"))
    ...
    if repetition_spatial_metrics_xgb:
        ...
        xgb_spatial_summary = xgb_spatial_df.drop(columns=['Repetition', 'encoder', 'model']).groupby('spatial_effect').agg(['mean', 'std', 'count'])
        xgb_spatial_df.to_csv(os.path.join(ENCODER_RESULTS_DIR, "xgb_spatial_metrics_all_reps.csv"), index=False)
        xgb_spatial_summary.to_csv(os.path.join(ENCODER_RESULTS_DIR, "xgb_spatial_metrics_summary.csv"))
    ...
    def aggregate_and_plot_surfaces(surface_list, true_surface, base_title, filename_prefix, model_name):
        if not surface_list:
        ...
        try:
            surfaces_stack = np.array(processed_surfaces) # Stack along a new axis (repetitions)
            ...
            np.save(os.path.join(ENCODER_RESULTS_DIR, f"{filename_prefix}_mean_surface.npy"), mean_surface)
            np.save(os.path.join(ENCODER_RESULTS_DIR, f"{filename_prefix}_std_surface.npy"), std_surface)
            ...
            plot_s(
                ...
                size=SIZE,
                title=[f"Mean {base_title} ({model_name}, {encoder})", f"Std Dev {base_title} ({model_name}, {encoder})", f"True {base_title}"],
                filename=f"{filename_prefix}_mean_std_true.pdf",
                experiment_dir=ENCODER_RESULTS_DIR
            )
            
        except Exception as e:
            print(f"Error aggregating/plotting surfaces for {model_name} {base_title}: {e}")
            import traceback
            traceback.print_exc()


    # MLP Surfaces
    aggregate_and_plot_surfaces(all_rep_est_intercept_mlp, true_intercept_surface, "Intercept", f"mlp_{encoder}_intercept", "MLP")
    aggregate_and_plot_surfaces(all_rep_mlp_svc_x1_raw, true_svc_x1_surface, "SVC X1 (Raw)", f"mlp_{encoder}_svc_x1_raw", "MLP")
    aggregate_and_plot_surfaces(all_rep_mlp_svc_x2_raw, true_svc_x2_surface, "SVC X2 (Raw)", f"mlp_{encoder}_svc_x2_raw", "MLP")
    aggregate_and_plot_surfaces(all_rep_mlp_svc_x1_smooth, true_svc_x1_surface, "SVC X1 (Smooth)", f"mlp_{encoder}_svc_x1_smooth", "MLP")
    aggregate_and_plot_surfaces(all_rep_mlp_svc_x2_smooth, true_svc_x2_surface, "SVC X2 (Smooth)", f"mlp_{encoder}_svc_x2_smooth", "MLP")

    # XGBoost Surfaces
    aggregate_and_plot_surfaces(all_rep_est_intercept_xgb, true_intercept_surface, "Intercept", f"xgb_{encoder}_intercept", "XGBoost")
    aggregate_and_plot_surfaces(all_rep_xgb_svc_x1_raw, true_svc_x1_surface, "SVC X1 (Raw)", f"xgb_{encoder}_svc_x1_raw", "XGBoost")
    aggregate_and_plot_surfaces(all_rep_xgb_svc_x2_raw, true_svc_x2_surface, "SVC X2 (Raw)", f"xgb_{encoder}_svc_x2_raw", "XGBoost")
    aggregate_and_plot_surfaces(all_rep_xgb_svc_x1_smooth, true_svc_x1_surface, "SVC X1 (Smooth)", f"xgb_{encoder}_svc_x1_smooth", "XGBoost")
    aggregate_and_plot_surfaces(all_rep_xgb_svc_x2_smooth, true_svc_x2_surface, "SVC X2 (Smooth)", f"xgb_{encoder}_svc_x2_smooth", "XGBoost")
